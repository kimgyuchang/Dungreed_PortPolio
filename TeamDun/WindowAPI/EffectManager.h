...
public :
	...
	void release();
	void render(HDC hdc);
	Effect* AddEffect(float x, float y, string imgName, int animSpeed, int frameX = 0, int frameY = 0, bool isLoop = false, int alpha = 255, float angle = 0, float scaleX = 1, float scaleY = 1, bool isAlpha = false, bool useCamera = false, bool isFirstViewing = true, string effectSound = "", float damage = 0 );
