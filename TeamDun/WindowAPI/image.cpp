...
HBITMAP image::GetRotatedBitmap(HDC hdc, float angle)
{
	...
	HBITMAP hbmResult = CreateCompatibleBitmap(hdc, getFrameWidth(), getFrameHeight());		// 회전할 비트맵을 출력받을 메모리비트맵 핸들   
	...
	PatBlt(destDC, 0, 0, getFrameWidth(), getFrameHeight(), PATCOPY);							// 선택된 브러시로, 회전할 비트맵을 출력받을 DC에, 미리 색상을 채워 지움   
	...
	xform.eDx = (FLOAT)getFrameWidth() / 2.0f;                                           // 3행 1열 성분 설정 (X축 이동 성분)   
	xform.eDy = (FLOAT)getFrameHeight() / 2.0f;                                          // 3행 2열 성분 설정 (Y축 이동 성분)   

	SetWorldTransform(destDC, &xform);

	// 회전된 메모리DC에, 회전할 이미지를 출력   

	BitBlt(destDC, -(getFrameWidth()/2.0f), -(getFrameHeight()/2.0f), getFrameWidth(), getFrameHeight(), getMemDC(), 0, 0, SRCCOPY);
